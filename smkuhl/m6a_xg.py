...
from oxford nanopore. The model is an
XGBoost.
"""
...
class M6AXG(XGBClassifier):
    ...
    def fit_semisupervised(
        ...
        # reshape X data into 2 dimensions
        X_train = X_train.reshape(-1, 90)
        X_valid = X_valid.reshape(-1, 90)
        
        print(X_train.shape)
        print(y_train.shape)
        print(X_valid.shape)
        print(y_valid.shape)
        # train the model and evaluate on training and validation data
        evals = [(X_train, y_train), (X_valid, y_valid)]
        self.fit(X_train, y_train, eval_set=evals, verbose=verbose)
        
        results = self.evals_result()
        
        # save model
        self.save_model("testing/final_save_model.json")
        
        # save the tree dump to a file for easier interpretability
        tree_dump = self.get_booster().get_dump()
        
        with open("testing/final_xgboost_classifier_trees.txt", "w") as tree_file:
            for i, tree in enumerate(tree_dump):
                tree_file.write(f"Tree {i}:\n")
                tree_file.write(tree)
                tree_file.write("\n\n")
        ...
        # also save the results
        with open('testing/xg_results.json', "w") as json_file:
            json.dump(results, json_file)
