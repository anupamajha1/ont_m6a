...
def run(config_file, train_chem, smart_init):
    ...
    # intitialize XGBoost Classifier Model
    model = M6AXG(
        n_estimators=32,
        max_depth=3,
        learning_rate=0.01,
    ...
    # loop
    for i in range(11): #range(max_epochs):
        
        # print(X_init.shape)
        # print(y_init.shape)
        # Train the model
        
        X_train_2d = X_init.reshape(-1, 90)
        X_valid_2d = X_val.reshape(-1, 90)
        
        results = model.fit_semisupervised(
            X_train_2d,
            y_init,
            X_valid=X_valid_2d,
            y_valid=y_val,
            ...
            tree_dump_path=best_save_model,
            final_save_model=final_save_model
        )

        
        # evaluate the model
        valid_preds = model.predict(X_valid_2d)
        sklearn_ap = average_precision_score(y_val, valid_preds)
        ...
        y_score = model.predict(X_train_2d)
    ...
    print("Plotting!")
    plt.plot(range(len(val_ap)), val_ap)
    plt.savefig("precision.png")
    plt.clf()
    
    plt.plot(range(len(all_num_pos)), all_num_pos)
    plt.savefig("num_pos.png")
    plt.clf()
...
def main():
    ...
    print(f"Training a {args.train_chem} " f"semi-supervised XGBoost model.")
